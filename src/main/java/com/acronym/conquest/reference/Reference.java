package com.acronym.conquest.reference;

public class Reference {

    public static final String MODID = "conquest";
    public static final String NAME = "Conquest";
    static final String VERSION_MAJOR = "1";
    static final String VERSION_MINOR = "0";
    static final String VERSION_PATCH = "0";
    static final String MINECRAFT_VERSION = "1.10.2";
    public static final String BUILD_VERSION = MINECRAFT_VERSION + "-" + VERSION_MAJOR + "." + VERSION_MINOR + "." + VERSION_PATCH;
}
